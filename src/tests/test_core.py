...
def test_highlight_url_with_truncation():
    ...
    assert highlight_url('https://some.website.com/long/path/to_some_file.html',
                         12) == 'https://<strong>some...</strong>'
    assert highlight_url('https://some.website.com/search?q=python',
                         12) == 'https://<strong>some...</strong>'
    assert highlight_url('https://some.website.com/search?q=python&lang=en',
                         12) == 'https://<strong>some...</strong>'
    assert highlight_url('https://some.website.com/search?q=python#section',
                         12) == 'https://<strong>some...</strong>'

    # 2. Truncate the URL after the domain
    assert highlight_url('https://some.website.com/long/path/to_some_file.html',
                         30) == 'https://<strong>some.website.com</strong>/long/...'
    assert highlight_url('https://some.website.com/search?q=python',
                         31) == 'https://<strong>some.website.com</strong>/search...'
    assert highlight_url('https://some.website.com/search?q=python&lang=en',
                         36) == 'https://<strong>some.website.com</strong>/search?q=<strong>py...</strong>'
    assert highlight_url('https://some.website.com/search?lang=en&q=python#section',
                         36) == 'https://<strong>some.website.com</strong>/search?lang...'


def test_reduce_line_breaks():
    assert reduce_line_breaks(
        "Line1\n\n\nLine2\n\nLine3") == "Line1\nLine2\nLine3"
...
def test_truncate_string():
    assert truncate_string(
        "This is a long string that needs to be truncated", 20) == "This is a long strin..."
    assert truncate_string("Short string", 20) == "Short string"
    assert truncate_string("Exact length string!",
                           20) == "Exact length string!"


def test_highlight_substring():
    assert highlight_substring(
        "highlight", "This is a highlight test") == "This is a <strong>highlight</strong> test"
    assert highlight_substring(
        "test", "This is a test") == "This is a <strong>test</strong>"
    assert highlight_substring(
        "notfound", "This is a test") == "This is a test"


def test_highlight_query_param():
    assert highlight_query_param(
        "q", "q=python&lang=en") == "q=<strong>python</strong>&lang=en"
    assert highlight_query_param(
        "lang", "q=python&lang=en") == "q=python&lang=<strong>en</strong>"
    assert highlight_query_param(
        "notfound", "q=python&lang=en") == "q=python&lang=en"
